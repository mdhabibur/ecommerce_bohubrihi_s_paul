...
const initPayment = async (req, res) => {
    ...
    const totalPrice = cartItems.map((cartItem) => cartItem.price).reduce((previous, current) => previous + current, 0)

    const totalItems = cartItems.map((cartItem) => cartItem.count).reduce((previous, current) => previous + current, 0)
...
module.exports = { initPayment, ipnHandler };
